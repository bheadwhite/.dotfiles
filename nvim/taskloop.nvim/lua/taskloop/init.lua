...
local cfg = { prefix = "<C-A-", tail = "split" } -- tail = "split" | "float"
...
local function save_clip_image(name)
  ...
  local helper = (os.getenv("HOME") or "~") .. "/.claude/taskloop-clip2png.sh"
...
local function ensure_daemon()
  local helper = (os.getenv("HOME") or "~") .. "/.claude/task-loop-ensure2.sh"
...
function M.prune()
  local helper = (os.getenv("HOME") or "~") .. "/.claude/taskloop-prune.py"
