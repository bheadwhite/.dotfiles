...
function M.set_default(client, bufnr)
  ...
  buf_set_keymap('n', '<leader>tt', "<cmd>lua require'lspsaga.provider'.lsp_finder()<CR>", 'lsp', 'lsp_definitions_references', 'Find definitions and references')
    -- buf_set_keymap('n','<leader>tt', '<cmd>lua vim.lsp.buf.type_definition()<CR>',  'lsp', 'lsp_definitions_references', 'Find definitions and references')
  ...
  if cap.definitionProvider then
    ...
    buf_set_keymap('n', '<F1>', '<cmd>lua vim.lsp.buf.definition()<CR>', 'lsp', 'lsp_preview_definition_saga', 'Preview definition')
  ...
  if cap.implementationProvider then
    buf_set_keymap('n','<leader>vi', '<cmd>lua vim.lsp.buf.implementation()<CR>', 'lsp', 'lsp_goto_implementation', 'Go to implementation')
  end
  if cap.referencesProvider then
    -- buf_set_keymap('n','<leader>tr', '<cmd>lua vim.lsp.buf.references()<CR>', opts)
    buf_set_keymap('n','<leader>vrr', "<cmd>lua require('telescope.builtin').lsp_references()<CR>", 'lsp', 'lsp_references', 'Show references')
  end

  -- buf_set_keymap('n','<leader><F14>', '<cmd>lua vim.lsp.buf.hover()<CR>',  'lsp', 'lsp_hover_docs', 'Hover documentation')
  buf_set_keymap('n', '<leader><F1>', "<cmd>lua require('lspsaga.hover').render_hover_doc()<CR>", 'lsp', 'lsp_hover_docs', 'Hover documentation')
  ...
  buf_set_keymap('n', '<leader>vsi', "<cmd>lua require('lspsaga.signaturehelp').signature_help()<CR>", 'lsp', 'lsp_signature_help', 'Show signature')
  ...
  -- buf_set_keymap('n','<leader>fe', '<cmd>:LspDiagnostics 0<CR>', opts)
   buf_set_keymap('n','<leader>vd', "<cmd>lua require('lsp.utils.functions').show_diagnostics()<CR>", 'lsp', 'lsp_show_diagnostics', 'Show diagnostics')
  buf_set_keymap('n','<leader>vld', '<cmd>lua vim.lsp.diagnostic.show_line_diagnostics()<CR>',  'lsp', 'lsp_show_line_diagnostics', 'Show line diagnostics' )
  -- buf_set_keymap('n','<leader>vld', "<cmd>lua require'lspsaga.diagnostic'.show_line_diagnostics()<CR>", 'lsp', 'lsp_show_line_diagnostics', 'Show line diagnostics')
  buf_set_keymap('n','<leader>fp', '<cmd>lua vim.lsp.diagnostic.goto_prev()<CR>', 'lsp', 'lsp_previous_diagnostic', 'Previous diagnostic')
  buf_set_keymap('n','<leader>fn', '<cmd>lua vim.lsp.diagnostic.goto_next()<CR>', 'lsp', 'lsp_next_diagnostic', 'Next diagnostic')
  -- buf_set_keymap('n', 'H', "<cmd>lua require'lspsaga.diagnostic'.lsp_jump_diagnostic_prev()<CR>", 'lsp', 'lsp_previous_diagnostic', 'Previous diagnostic')
  -- buf_set_keymap('n', 'L', "<cmd>lua require'lspsaga.diagnostic'.lsp_jump_diagnostic_next()<CR>", 'lsp', 'lsp_next_diagnostic', 'Next diagnostic');
