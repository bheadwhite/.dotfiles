...
HINTS = ("Follow the standing hints: use the project's OWN design tokens, styling, and "
         ...
         "selection/caret, races, anything that only shows in the browser) — ESPECIALLY when "
         "feedback says your fix didn't work: do NOT re-guess. INSTRUMENT with a copy-paste-"
         "friendly console probe and ask the user to run it. Convention (the user pastes the "
         "result straight back, so it must survive copy-paste): install once, guarded — "
         "`if(!window.__probe){window.__probe=(l)=>{try{console.groupEnd()}catch{}; "
         "console.groupCollapsed('[probe] '+(l||'')+' @'+new Date().toLocaleTimeString())}}` — "
         "then at each suspect point `console.log(JSON.stringify({…}))` (STRINGIFY — raw objects "
         "render as expandos that don't paste). The user calls `__probe('<scenario>')` in the "
         "console right before reproducing, so that attempt's logs nest in ONE collapsible group "
         "they expand + copy. In 'Verify by' tell them exactly: call `__probe('<scenario>')`, "
         "reproduce, paste the '[probe]' group back as a rework note. If it's an ON-LOAD issue "
         "(no user action), call `__probe('on load')` yourself at init so it captures on refresh. "
         "Next round, fix from their paste and REMOVE the probe. "
